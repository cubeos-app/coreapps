...
func (h *HALHandler) GetSavedWiFiNetworks(w http.ResponseWriter, r *http.Request) {
	...
	output, err := exec.Command("nmcli", "-t", "-f", "NAME,TYPE,AUTOCONNECT", "connection", "show").Output()
	if err == nil {
	...
	// Try wpa_cli
	output, err = exec.Command("wpa_cli", "-i", iface, "list_networks").Output()
	if err == nil {
...
// @Failure 400 {object} ErrorResponse "SSID required"
...
func (h *HALHandler) ForgetWiFiNetwork(w http.ResponseWriter, r *http.Request) {
	...
	// Try NetworkManager first (nmcli)
	err = exec.Command("nmcli", "connection", "delete", ssid).Run()
	if err == nil {
		jsonResponse(w, http.StatusOK, map[string]interface{}{
			"success": true,
			"ssid":    ssid,
	...
	output, err := exec.Command("wpa_cli", "-i", iface, "list_networks").Output()
	if err != nil {
		errorResponse(w, http.StatusInternalServerError, "failed to list networks: "+err.Error())
		return
	}
	...
	if networkID == "" {
		errorResponse(w, http.StatusNotFound, "network not found: "+ssid)
		return
	}

	// Remove the network
	err = exec.Command("wpa_cli", "-i", iface, "remove_network", networkID).Run()
	if err != nil {
		errorResponse(w, http.StatusInternalServerError, "failed to remove network: "+err.Error())
		return
	}

	// Save configuration
	exec.Command("wpa_cli", "-i", iface, "save_config").Run()
